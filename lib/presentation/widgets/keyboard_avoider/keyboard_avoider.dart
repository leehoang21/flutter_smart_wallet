...
/// If [autoScroll] is true and the [child] contains a focused widgets such as a [TextField],
...
class KeyboardAvoider extends StatefulWidget {
  ...
  /// Whether to auto-scroll to the focused widgets after the keyboard appears. Defaults to false.
  /// Could be expensive because it searches all the child objects in this widgets's render tree.
  final bool autoScroll;

  /// Space to put between the focused widgets and the top of the keyboard. Defaults to 12.
  /// Useful in case the focused widgets is inside a parent widgets that you also want to be visible.
  ...
  })  : assert(child is ScrollView ? child.controller != null : true, 'Error'),
        super(key: key);
...
class _KeyboardAvoiderState extends State<KeyboardAvoider>
  ...
  Widget build(BuildContext context) {
    ...
    // it possible to scroll to the focused widgets.
  ...
  void _resize() {
    // Calculate Rect of widgets on screen
    ...
    // If widgets is entirely covered by keyboard, do nothing
    if (widgetRect.top > keyboardTop) {
      return;
    }

    // If widgets is partially obscured by the keyboard, adjust bottom padding to fully expose it
  ...
  void _scrollToFocusedObject() {
    final _renderObject = context.findRenderObject();
    if (_renderObject != null) {
      final focused = _findFocusedObject(_renderObject);
      if (focused != null) {
